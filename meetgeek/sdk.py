...
def maybe_fail(meeting):
    """
        Each activity invocation independently throws with probability failure_rate.
        Simulates transient/flaky failures (which retries usually recover from).
    """
    rate = meeting.get("failure_rate", 0.0)
    if rate and random.random() < rate:
        raise RuntimeError(f"random failure (p={rate}): {meeting.get('title')!r}")
...
class MeetGeekSDK:
    ...
    def run_transcript(cls, meeting):
        maybe_fail(meeting)
        time.sleep(duration(meeting, TRANSCRIBE))

    @classmethod
    def run_speaker_diarization(cls, meeting):
        maybe_fail(meeting)
        time.sleep(duration(meeting, DIARIZE))

    @classmethod
    def run_language_identification(cls, meeting):
        maybe_fail(meeting)
        time.sleep(duration(meeting, LANGUAGE))

    @classmethod
    def run_silence_intervals(cls, meeting):
        maybe_fail(meeting)
        time.sleep(duration(meeting, SILENCE))

    # Analysis Mock (OpenAI Responses API)
    @classmethod
    def run_update_vector_store(cls, meeting):
        maybe_fail(meeting)
        time.sleep(api_duration(meeting, VECTOR_STORE))

    @classmethod
    def run_infer_unknown_speakers(cls, meeting):
        maybe_fail(meeting)
        time.sleep(api_duration(meeting, INFER_SPEAKERS))

    @classmethod
    def run_template(cls, meeting):
        maybe_fail(meeting)
        time.sleep(api_duration(meeting, TEMPLATE))

    @classmethod
    def run_summary(cls, meeting):
        maybe_fail(meeting)
        time.sleep(api_duration(meeting, SUMMARY))

    @classmethod
    def run_topics_and_highlights(cls, meeting):
        maybe_fail(meeting)
        time.sleep(api_duration(meeting, TOPICS))

    @classmethod
    def run_keyword_highlights(cls, meeting):
        maybe_fail(meeting)
        time.sleep(api_duration(meeting, KEYWORDS))

    @classmethod
    def run_kpis(cls, meeting):
        maybe_fail(meeting)
        time.sleep(api_duration(meeting, KPIS))

    @classmethod
    def run_kpis_summary(cls, meeting):
        maybe_fail(meeting)
        time.sleep(api_duration(meeting, KPIS_SUMMARY))

    @classmethod
    def run_meeting_workflows(cls, meeting):
        maybe_fail(meeting)
        time.sleep(api_duration(meeting, MEETING_WORKFLOWS))
...
class AsyncMeetGeekSDK:
    ...
    async def run_transcript(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(duration(meeting, TRANSCRIBE))

    @classmethod
    async def run_speaker_diarization(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(duration(meeting, DIARIZE))

    @classmethod
    async def run_language_identification(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(duration(meeting, LANGUAGE))

    @classmethod
    async def run_silence_intervals(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(duration(meeting, SILENCE))

    # Analysis Mock (OpenAI Responses API)
    @classmethod
    async def run_update_vector_store(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(api_duration(meeting, VECTOR_STORE))

    @classmethod
    async def run_infer_unknown_speakers(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(api_duration(meeting, INFER_SPEAKERS))

    @classmethod
    async def run_template(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(api_duration(meeting, TEMPLATE))

    @classmethod
    async def run_summary(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(api_duration(meeting, SUMMARY))

    @classmethod
    async def run_topics_and_highlights(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(api_duration(meeting, TOPICS))

    @classmethod
    async def run_keyword_highlights(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(api_duration(meeting, KEYWORDS))

    @classmethod
    async def run_kpis(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(api_duration(meeting, KPIS))

    @classmethod
    async def run_kpis_summary(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(api_duration(meeting, KPIS_SUMMARY))

    @classmethod
    async def run_meeting_workflows(cls, meeting):
        maybe_fail(meeting)
        await asyncio.sleep(api_duration(meeting, MEETING_WORKFLOWS))
